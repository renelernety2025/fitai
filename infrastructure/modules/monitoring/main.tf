...
# Web service CPU/memory (previously only api was monitored)
resource "aws_cloudwatch_metric_alarm" "web_cpu" {
  alarm_name          = "${var.project_name}-${var.env}-web-cpu-high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  metric_name         = "CPUUtilization"
  namespace           = "AWS/ECS"
...
resource "aws_cloudwatch_metric_alarm" "web_memory" {
  alarm_name          = "${var.project_name}-${var.env}-web-memory-high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  metric_name         = "MemoryUtilization"
  namespace           = "AWS/ECS"
...
# p99 latency on the API target group (dashboard had avg only, no alarm)
resource "aws_cloudwatch_metric_alarm" "api_p99_latency" {
  alarm_name          = "${var.project_name}-${var.env}-api-p99-latency"
  comparison_operator = "GreaterThanThreshold"
...
# Per-target-group 5XX (previously only ELB-level 5XX was alarmed, which
# misses app errors and can't attribute api vs web)
resource "aws_cloudwatch_metric_alarm" "api_tg_5xx" {
  alarm_name          = "${var.project_name}-${var.env}-api-target-5xx"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
...
resource "aws_cloudwatch_metric_alarm" "web_tg_5xx" {
  alarm_name          = "${var.project_name}-${var.env}-web-target-5xx"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  metric_name         = "HTTPCode_Target_5XX_Count"
  namespace           = "AWS/ApplicationELB"
...
# Crash-loop detector: running task count drops to 0 (Container Insights)
resource "aws_cloudwatch_metric_alarm" "api_tasks_low" {
  alarm_name          = "${var.project_name}-${var.env}-api-no-running-tasks"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = 2
...
resource "aws_cloudwatch_metric_alarm" "web_tasks_low" {
  alarm_name          = "${var.project_name}-${var.env}-web-no-running-tasks"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = 2
  metric_name         = "RunningTaskCount"
  namespace           = "ECS/ContainerInsights"
