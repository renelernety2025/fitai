...
# ── VPC endpoints (2026-07 audit) ────────────────────────────────────
# S3 gateway endpoint is free and removes S3 traffic from the NAT path;
# interface endpoints keep ECR pulls / secrets / logs working even if the
# single NAT gateway's AZ fails, and cut NAT data-processing cost.
