...
export default function ProgresFotkyPage() {
  ...
  useEffect(() => { document.title = 'FitAI — Progress Photos'; }, []);
      ...
      <>
        ...
        <section className="mb-16 pt-12">
          ...
          <p className="max-w-2xl text-base leading-relaxed text-white/55">
            Your private progress photo gallery. Optional AI body composition analysis. Photos are yours only — no admin access, no sharing without your consent.
          </p>
        </section>
        ...
        {stats && (
          <section className="mb-20 grid grid-cols-2 gap-6 border-y border-white/10 py-12 sm:grid-cols-4">
            <Stat value={stats.total} label="Photos" />
            <Stat value={stats.daysTracked} label="Days tracked" />
            <Stat value={stats.byAngle.front} label="Front" />
            <Stat value={stats.byAngle.side + stats.byAngle.back} label="Side + Back" />
          </section>
        )}
        ...
        <section className="mb-20">
          ...
          />
          {busy && <div className="mt-4 text-xs text-white/40">Uploading...</div>}
        </section>
        ...
        {compareA && compareB && (
          <section className="mb-20">
            <div className="mb-3 text-[10px] font-semibold uppercase tracking-[0.25em] text-white/40">
              Comparison · {new Date(compareA.takenAt).toLocaleDateString('en-US')} ↔ {new Date(compareB.takenAt).toLocaleDateString('en-US')}
            </div>
        ...
        <section className="mb-32">
          ...
          {photos.length === 0 ? (
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-12 text-center text-white/40">
              No photos yet. Start by uploading your first.
            </div>
          ) : (
...
function PhotoCard({
  ...
  const sideLabel = { FRONT: 'Front', SIDE: 'Side', BACK: 'Back' }[photo.side];
          ...
          {photo.analysis.estimatedBodyFatPct != null ? (
            <>
              <div className="font-semibold text-[#A8FF00]">
                ~{photo.analysis.estimatedBodyFatPct.toFixed(1)}% body fat
              </div>
              {photo.analysis.estimatedMuscleMass && (
                <div className="text-white/55">Muscle mass: {photo.analysis.estimatedMuscleMass}</div>
              )}
            </>
          ) : (
            <div className="text-[#FF9F0A]" title={photo.analysis.postureNotes || ''}>
              ⚠ AI could not analyze — {photo.analysis.postureNotes?.slice(0, 60) || 'unsuitable image'}
            </div>
          )}
        </div>
      )}
