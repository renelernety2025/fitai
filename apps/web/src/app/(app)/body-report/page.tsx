...
const MUSCLES = [
  { name: 'Chest', volume: 18, status: 'recovered' },
  { name: 'Back', volume: 24, status: 'fresh' },
  { name: 'Shoulders', volume: 12, status: 'due' },
  { name: 'Legs', volume: 28, status: 'fresh' },
  { name: 'Biceps', volume: 9, status: 'recovered' },
  { name: 'Triceps', volume: 11, status: 'recovered' },
  { name: 'Core', volume: 15, status: 'fresh' },
  { name: 'Calves', volume: 6, status: 'due' },
];

const INSIGHTS = [
  { icon: '\u2197', text: 'Bench press progress: +2.5kg in the last 2 weeks. Consistent growth.', type: 'positive' as const },
  { icon: '\u26A0', text: 'Shoulders have 14 sessions since last deload. Recommend a recovery day.', type: 'warning' as const },
  { icon: '\u25CE', text: 'Your sleep improved by 12% — directly correlates with better form.', type: 'neutral' as const },
];
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '0 16px 80px', opacity: loaded ? 1 : 0, transition: 'opacity .6s ease' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 32 }}>
        ...
        <Card padding={20}>
          <Metric label="Active Streak" value="47" unit="days" />
        </Card>
      </div>
      ...
      <p className="v3-caption" style={{ textAlign: 'center', color: 'var(--text-3)', marginTop: 40 }}>
        Generated by FitAI {'\u00B7'} {now.toLocaleDateString('en-US')} {'\u00B7'} W{weekNum}
      </p>
    </div>
  );
}
