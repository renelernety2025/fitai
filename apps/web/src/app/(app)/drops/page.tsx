...
import type { DropItem, DropPurchaseWithDrop } from '@fitai/shared';

type Drop = DropItem;
type Purchase = DropPurchaseWithDrop;
...
export default function DropsPage() {
  ...
  useEffect(() => {
    Promise.all([getDrops(), getMyDropPurchases()]).then(([d, p]) => { setDrops(d); setPurchases(p); }).catch(() => {}).finally(() => setLoading(false));
  }, []);

  function handlePurchase(id: string) {
    setPurchasing(id);
    purchaseDrop(id).then(() => Promise.all([getDrops(), getMyDropPurchases()]).then(([d, p]) => { setDrops(d); setPurchases(p); })).catch(() => {}).finally(() => setPurchasing(null));
  }
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 16 }}>
              {drops.map((drop) => {
                const soldOut = drop.remainingEditions <= 0;
                const pct = ((drop.totalEditions - drop.remainingEditions) / Math.max(drop.totalEditions, 1)) * 100;
                return (
                    ...
                    <div style={{ padding: 20 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16 }}>
                        <div>
                          ...
                          <div className="v3-caption" style={{ color: 'var(--text-3)' }}>{drop.remainingEditions}/{drop.totalEditions} left</div>
                        </div>
                  ...
                  {purchases.map((p) => (
                    <Card key={p.id} padding="12px 16px">
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                          <span className="v3-body" style={{ fontWeight: 600, color: 'var(--text-1)' }}>{p.drop.name}</span>
