/**
 * Rep counter — mobile port from apps/web/src/lib/rep-counter.ts
 * State machine: START → ECCENTRIC → HOLD → CONCENTRIC → START = 1 rep
 * Runs in JS thread (not worklet) because uses closure state.
 * Frame processor should call processFrame via runOnJS.
 */

import type { PoseLandmarks, ExercisePhaseDefinition, RepDataEntry } from './types';
...
export function createRepCounter(phases: ExercisePhaseDefinition[]) {
  ...
  function processFrame(landmarks: PoseLandmarks, timestampMs: number): RepFrameResult {
    ...
    if (
      ...
      if (nextIdx === 0 && state.currentPhase !== 0) {
        ...
        const avgScore = state.currentRepScores.length
          ? Math.round(
              state.currentRepScores.reduce((a, b) => a + b, 0) /
                state.currentRepScores.length,
            )
          : 0;
    ...
    // Backward movement → partial rep flag
