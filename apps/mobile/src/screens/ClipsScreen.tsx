...
export function ClipsScreen() {
  ...
  function handleLike(id: string, currentlyLiked: boolean) {
    haptic.tap();
  ...
  return (
    <V2Screen>
      ...
      {clips === null ? (
        <LoadingState label="Loading clips" />
      ) : items.length === 0 ? (
        <EmptyState icon="🎬" title="No clips yet" body="Short videos from the community will appear here as they roll in." />
      ) : null}
      ...
      {items.length > 0 && (
        ...
        >
          <Text style={{ color: v2.green, fontSize: 13, fontWeight: '600' }}>
