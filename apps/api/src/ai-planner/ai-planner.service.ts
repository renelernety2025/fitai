...
export class AIPlannerService {
  ...
  async generatePlan(userId: string) {
    ...
    const availableExercises = profile.hasGymAccess
      ? allExercises
      : allExercises.filter((e) => HOME_EXERCISES.includes(e.name) || (e.muscleGroups as string[]).includes('CORE'));
  ...
  async getBreakRecoveryPlan(userId: string) {
    const progress = await this.prisma.userProgress.findUnique({ where: { userId } });
    if (!progress?.lastWorkoutDate) return null;
  ...
  async getAsymmetryReport(userId: string) {
    const history = await this.prisma.exerciseHistory.findMany({
      where: { userId },
      orderBy: { date: 'desc' },
      take: 100,
    ...
    const jointIssues = new Map<string, number>();
    events.forEach((e) => {
      jointIssues.set(e.jointName, (jointIssues.get(e.jointName) || 0) + 1);
    });

    const asymmetries: { joint: string; count: number; recommendation: string }[] = [];
    ...
    const earlySetScores: number[] = [];
    const lateSetScores: number[] = [];
    recentSessions.forEach((s) => {
      if (s.status !== 'COMPLETED') return;
      if (s.setNumber <= 2) earlySetScores.push(s.formScore);
      else lateSetScores.push(s.formScore);
    });
  ...
  async getHomeAlternative(userId: string) {
    const profile = await this.getOrCreateProfile(userId);
  ...
  private buildPlanPrompt(ctx: any): string {
...
${ctx.availableExercises.map((e: any) => `- ${e.id}: ${e.nameCs} (${(e.muscleGroups as string[]).join(', ')})`).join('\n')}

${ctx.safetyEvents.length > 0 ? `Časté safety problémy: ${[...new Set(ctx.safetyEvents.map((e: any) => e.jointName))].join(', ')} — vyhni se cviků co zatěžují tyto klouby.` : ''}
...
- Při zranění vynech cviky co zatěžují danou oblast`;
  ...
  private async createPlanFromAI(userId: string, planJson: any, profile: any) {
    return this.prisma.workoutPlan.create({
      data: {
        ...
        days: {
          create: (planJson.days || []).map((day: any, i: number) => ({
            dayIndex: i,
