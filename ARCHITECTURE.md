...
| 20 | 2026-05-11 | API convention enforcement v CI | `scripts/check-api-conventions.sh` greps controllers for raw `@Body()` types, missing `@Throttle()` on AI endpoints, unknown throttle keys. Spuštěno v `.github/workflows/ci.yml` jako required PR step + `npm run lint:conventions`. Intentional exceptions v `scripts/check-api-conventions.ignore`. Doplňuje 6-slice audit (2026-05-11) — zabraňuje znovuobjevení stejných findings při budoucích PRs |
| 21 | 2026-05-11 | Atomic XP debit pattern přes `updateMany` s `totalXP gte` filtrem | Pro každou XP-spending operaci (creator subscribe, creator tip, marketplace purchase, experiences book): `userProgress.updateMany({ where: { userId, totalXP: { gte: cost } }, data: { totalXP: { decrement: cost } } })` + check `count === 0` → BadRequestException. Eliminuje race condition kde concurrent requests obě projdou balance check a obě decrement |
| 22 | 2026-07-02 | `prisma migrate deploy` nahrazuje `db push --accept-data-loss` (supersedes ADR-2) | Baseline `0_init` vygenerován z schema.prisma (`migrate diff --from-empty`), na produkci označen `migrate resolve --applied` (zápis jen do `_prisma_migrations`). Migrate ECS task nově spouští `prisma migrate deploy`. Důvody: reviewable DDL v PR, žádný tichý DROP (db push s `--accept-data-loss` mazal sloupce bez potvrzení), CI guard `check-destructive-migration.sh` + pre-migrate RDS snapshot, dohledatelná historie. Trade-off: každá schema změna vyžaduje committed migraci (`migrate dev`). Rollback cutoveru = repin `fitai-migrate:3` (revize zůstává ACTIVE); `0_init` řádek je pod db push inertní. Lokální dev: `docker compose down -v` + `migrate dev` replay |
| 23 | 2026-07-02 | Centrální `ClaudeService` (@Global) místo per-service Anthropic klientů | 13 služeb × `new Anthropic` + 27 hardcoded model stringů → jeden wrapper: model registry (`CLAUDE_MODELS`: haiku/sonnet/opus), 1 retry na 429/5xx, CloudWatch usage metriky centrálně (streamy získaly tracking, který nikdy neměly), opt-in Redis response cache (`cacheKey` + TTL — zapíná se per-call po review, ne plošně). `complete()` vrací text a hází při selhání — každý caller si drží svůj domain fallback. Migrace modelu = 1 řádek |
| 24 | 2026-07-02 | `@fitai/shared/src/api/*` jako API contract layer | Response typy odvozené ze skutečných NestJS service returns + Prisma modelů, sdílené web ↔ (postupně) mobile. První 3 domény: cross-industry (44 typů), marketplace (47), gamification. Nahrazuje ruční `Promise<any>` (−100+ výskytů) a manuální CONTRACTS.md policing. Typování odhalilo 11+ pre-existujících kontrakt bugů na stránkách (označeno `TODO(shared-types)`) — typy reflektují API realitu, stránky se opravují postupně. Json blobs = `unknown`, nikdy `any` |
