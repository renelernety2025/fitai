...
| 18 | 2026-05-04 | Wearable provider OAuth via JWT-signed state | OAuth `state` parametr je signed JWT (`expiresIn: '10m'`) embedding `userId + provider`. Eliminates need pro Redis state store. `/callback` je veřejný endpoint (Oura redirect bez user JWT), bezpečí přes signature ověření. Tokens v `WearableConnection.accessToken/refreshToken` plaintext — TODO: KMS encryption when production-ready |
